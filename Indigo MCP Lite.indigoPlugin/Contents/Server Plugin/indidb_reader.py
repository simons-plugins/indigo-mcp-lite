...
The parse is cached keyed on the file's (mtime_ns, size); reuse until
the file changes. A mid-write parse failure (Indigo rewrites the file
periodically) raises a friendly ValueError with a retry hint rather
...
def _parse_action(action_el):
    ...
    if cls in (_CLASS_DEVICE, _CLASS_DEVICE_UNIVERSAL):
        ...
        return {
            ...
            "action_value": _int(action_el, "DeviceActionValue"),
        }
    if cls == _CLASS_EXEC_GROUP:
        return {
            "type": "execute_action_group",
            "class": cls,
            "action_group_id": _int(action_el, "ActionGroupID"),
        }
    if cls == _CLASS_VARIABLE:
        code = _int(action_el, "VarAction")
        return {
            "type": "variable_action",
            "class": cls,
            "variable_id": _int(action_el, "VarID"),
            "action_code": code,
    ...
    if cls == _CLASS_SCRIPT:
        ...
        script_type = _int(action_el, "ScriptType")
        return {
    ...
    if cls == _CLASS_HVAC:
        code = _int(action_el, "HVACAction")
...
def _parse_steps(owner_el):
    ...
    for action_el in steps_el.findall("Action"):
        step = _parse_action(action_el)
        if step is not None:
            steps.append(step)
    return steps


def _parse_condition_leaf(cond_el):
    ...
    """
    type_code = _int(cond_el, "Type")
    if _text(cond_el, "DevID") is not None:
        comp = _int(cond_el, "DevComp")
        leaf = {
            "type": "device_state",
            "type_code": type_code,
            "device_id": _int(cond_el, "DevID"),
    ...
    if _text(cond_el, "VarID") is not None:
        ...
        comp = _int(cond_el, "VarState")
        leaf = {
            "type": "variable",
            "type_code": type_code,
            "variable_id": _int(cond_el, "VarID"),
            "comparator_code": comp,
            "comparator": _COMPARATOR_LABELS.get(comp),
            "value": _text(cond_el, "VarValue"),
        }
        variable_id2 = _int(cond_el, "VarID2")
...
def _parse_condition(cond_el):
    """``Condition`` element (wrapper or leaf) → nested dict or None."""
    nested = cond_el.find("ConditionList")
    if nested is not None:
        return _parse_condition_list(nested)
    return _parse_condition_leaf(cond_el)


def _parse_trigger_watch(trigger_el):
...
def _parse_automation(tag, el):
    """One top-level TDTrigger/Trigger/ActionGroup element → record.

    Returns None when the element has no parseable integer ID —
    anonymous embedded ActionGroups never reach here (depth tracking),
    but the guard also protects against future schema drift.
    """
    ...
    record = {
        ...
        "folder_id": _int(el, "FolderID"),
        "steps": _parse_steps(el),
    }
    ...
    if condition_el is not None:
        record["conditions"] = _parse_condition(condition_el)
    else:
        record["conditions"] = None
    if tag == "Trigger":
        record["watch"] = _parse_trigger_watch(el)
    return record
...
class IndiDbReader:
    ...
    def automations(self):
        ...
        {id: record}}. Cached until the DB file's mtime/size change.
        Raises a friendly ValueError when the path is unavailable or
        the file can't be parsed (mid-write)."""
        path = self._db_path()
        try:
            stat = os.stat(path)
    ...
    def resolve_name(self, collection_attr, entity_id):
        ...
        callers keep the raw id either way."""
        ...
        try:
            for event, el in ET.iterparse(path, events=("start", "end")):
                ...
                if el.tag in tag_to_kind:
                    depth -= 1
                    if depth == 0:
                        record = _parse_automation(el.tag, el)
