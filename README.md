...
| `list_plugin_actions` | List one plugin's declared Actions.xml actions: id, name, callback method, whether it's hidden (uiPath="hidden" -- programmatic-only, not meant for menus), whether it requires a device, and its ConfigUI fields as props (id/type/label/default; a dynamic menu is flagged values:"dynamic" with its source callback since its legal values can't be read cross-plugin; a static menu lists its real values). Use plugin_execute_action to actually invoke one. |
